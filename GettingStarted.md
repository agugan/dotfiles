...
2. **`capture-prefs.sh -i`** — imports preferences exported from your previous machine, overriding the baseline where they overlap.

If you have not yet exported preferences from a previous machine, skip step 2 for now and run `capture-prefs.sh -i` once your old machine's export is available. See [Extras.md — osx-defaults.sh](Extras.md#osx-defaultssh) for full details.
